...
double cal_score(int playerID, int teammateID, int **mapStat, int **sheepStat, int mapSize){
    // init
    ...
    for(int i = 0; i < mapSize; i++){
        for(int j = 0; j < mapSize; j++){
            if((mapStat[i][j] == playerID || mapStat[i][j] == teammateID) && !visited[i][j]){
    ...
    for (int i = 0; i < mapSize; i++) {
        for(int j = 0; j < mapSize; j++){
            if((mapStat[i][j] == playerID || mapStat[i][j] == teammateID) && sheepStat[i][j] > 1){
                int live = 0;
                for (int k =0; k <8 ; k++){
                    int x = i + VALID_DIR[k][0];
                    int y = j + VALID_DIR[k][1];
