...
vector<vector<int>> get_actions(int playerID,int **mapStat,int **sheepStat, int mapSize, int ori_playerID) {
    int total_points = 0;
    for(int i = 0; i < mapSize; i++){
        for(int j = 0; j < mapSize; j++){
    ...
    if (total_points >= 16) return actions;
    if (ori_playerID == playerID){
        for(int i = 0; i < mapSize; i++){
            for(int j = 0; j < mapSize; j++){
            ...
            for(int j = 0; j < mapSize; j++){
                if(mapStat[i][j] == playerID){
                    for(int k = 0; k < 8; k++){
                        ...
                        int y = j + VALID_DIR[k][1];
                        if(x >= 0 && x < mapSize && y >= 0 && y < mapSize && mapStat[x][y] == 0){
                            actions.push_back({i, j, 0, VALID_DIR[k][2]});
                        }
                    }
                }
            }
...
bool is_terminal(int **mapStat,int **sheepStat, int mapsize, vector<int> players, int ori_playerID){
    for(int i = 0; i < 4; i++){
        auto actions = get_actions(players[i], mapStat, sheepStat, mapsize, ori_playerID);
        if (actions.size() > 0) {
            return false;
        }
    }
